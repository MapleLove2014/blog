---
title: HashMap put源码分析
author: ZhangMapler
date: 2020-04-03 15:19:00 +0800
categories: [Java]
tags: [源码]
math: false
---

# JDK1.8 HashMap put方法及扩容源码详细分析
...
最后遍历桶中的每个节点（跳过第一个，因为已经判断过了）。如果已经到桶的最后一个节点还没有找到此key，表示桶中不存在此节点。此时创建一个新节点链接到桶的末尾（**尾插法**），然后判断桶是否需要树化，注意这里判断条件binCount是根据插入前的桶大小判断的。如果在第二个if中找到已经存在key的节点，则退出。仍然维持了invariant。
...
接着往下看，如果已经存在此key的节点。通过onlyIfAbsent判断是否需要更新值。因为这是对节点的Access，为了维持LinkedHashMap的访问顺序，这里会进行回调。最后返回旧值给调用方。因为这里没有插入新元素，故也不需要更新map的size，直接进行返回即可。
...
5. 顺便提一下HashTable，用的也不多，不单门搞一篇说这个了

    1. 不允许null作为key和value
    2. 通过synchronized保证线程安全
    3. 不采用2的n次方作为桶数组大小
    4. 分片采用模的方式

## 总结

在这篇文章中，详细分析了HashMap的put源码，从源码的角度展现了HashMap添加或更新元素的核心流程：
1. 经过扰乱的key的hash值通过与的方式定位到桶
2. 遍历桶中的节点，如果存在当前key，则进行更新。否则进行尾插当前节点
3. 回调LinkedHashMap用以维持插入顺序和访问顺序

和扩容的核心流程：
1. 初始化新的桶空间
2. 遍历每一个桶将起节点根据高位hash值分为两类
3. 根据桶定位方式，将两类节点放到新的桶空间中

最后对于HashMap的一些设计思想和面试的问题，做出了尝试的解答。后面还会有一篇LinkedHashMap的文章，相信能更加加深读者的理解。